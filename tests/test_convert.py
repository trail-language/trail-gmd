...
def test_panel_shape_keys_and_dtypes(gmd_csv):
    ...
    assert panel.columns[:2] == ["entity", "time"]
    assert set(panel.columns) == {"entity", "time", "gmd.rGDP", "gmd.infl", "gmd.income_group"}
    assert panel.schema["entity"] == pl.Utf8
    ...
    assert sorted(panel["entity"].unique().to_list()) == ["DEU", "USA"]
...
def test_historical_only_masks_projection_cells(gmd_csv):
    ...
    usa22 = masked.filter((pl.col("entity") == "USA") & (pl.col("time").dt.year() == 2022)).to_dicts()[0]
...
def test_include_forecasts_when_disabled(gmd_csv):
    ...
    usa22 = full.filter((pl.col("entity") == "USA") & (pl.col("time").dt.year() == 2022)).to_dicts()[0]
...
def test_unknown_and_absent_fields_are_ignored(gmd_csv):
    ...
    assert panel.columns == ["entity", "time", "gmd.rGDP"]
